...
if CLIENT then
	...
	surface.CreateFont("nut_CA_Whisper", {
		font = mainFont,
		size = 16,
		weight = 1000,
		antialias = true,
	...
	surface.CreateFont("nut_CA_Yell", {
		font = mainFont,
		size = 23,
		weight = 1000,
		antialias = true,
...
do
	...
	nut.chat.Register("whisper", {
		...
		onChat = function(speaker, text)
			chat.AddText(Color(r - 25, g - 25, b - 25), nut.schema.Call("GetPlayerName", speaker, "whisper", text)..' says, "'..text..'"')
		end,
		prefix = {"/w", "/whisper"},
		font = "nut_C_Whisper"
	})

	nut.chat.Register("name", {
		canhear = nut.config.chatRange,
		onChat = function(speaker, text)
			chat.AddText(Color(r, g, b), hook.Run("GetPlayerName", speaker, "name", text)..' says, "'..speaker:Name()..' '..text..'"')
		end,
		prefix = "/이름",
		noSpacing = true
	})

	--crt, Correction commands. : If there is grammatical error or something wrong about contents, use this command
	nut.chat.Register("crt", {
		canHear = nut.config.chatRange,
		onChat = function(speaker, text)
			chat.AddText(Color(234, 131, 87), "", speaker, " *"..text)
		end,
		prefix = {"/miss", "/crt", "/c", "/미스"},
	...
	nut.chat.Register("q", {
		onChat = function(speaker, text)
			chat.AddText(Color(120, 30, 70), "[Q] ", Color(210, 210, 210), speaker:Name(), Color(120, 30, 70), ": "..text)
		end,
		deadCanTalk = true,
		prefix = {"/q", "/question"}
	})
	...
	nut.chat.Register("a", {
		onChat = function(speaker, text)
			chat.AddText(Color(22, 120, 73), "[A] ", Color(90, 90, 90), speaker:Name(), Color(22, 120, 73), ": "..text)
		end,
		deadCanTalk = true,
		prefix = {"/a", "/answer"}
	})
	...
	nut.chat.Register("ic", {
		canHear = nut.config.chatRange,
		onChat = function(speaker, text)
			chat.AddText(Color(r, g, b), hook.Run("GetPlayerName", speaker, "ic", text)..' says, "'..text..'"')
		end
	})
	...
	nut.chat.Register("yell", {
		...
		onChat = function(speaker, text)
			chat.AddText(Color(r + 35, g + 35, b + 35), hook.Run("GetPlayerName", speaker, "yell", text)..' says, "'..text..'"')
		end,
		prefix = {"/y", "/yell"},
		font = "nut_C_Yell"
	})
	...
	nut.chat.Register("mel", {
		canHear = nut.config.yellRange,
		onChat = function(speaker, text)
			chat.AddText(Color(r, g, b), "****"..nut.schema.Call("GetPlayerName", speaker, "mel", text).." "..text)
		end,
		prefix = {"/mel", "/actionl", "/mey"},
		font = "nut_CA_Yell"
	})
	...
	nut.chat.Register("mec", {
		canHear = nut.config.whisperRange,
		onChat = function(speaker, text)
			chat.AddText(Color(r, g, b), "**"..nut.schema.Call("GetPlayerName", speaker, "mec", text).." "..text)
		end,
		prefix = {"/mec", "/actionc", "/mew", "/med"},
		font = "nut_CA_Whisper"
	})
	...
	nut.chat.Register("ooc", {
		...
		canSay = function(speaker)
			...
			nut.util.Notify(""..math.ceil(nextOOC - CurTime()).." 초 후에 OOC를 다시 사용할 수 있습니다.", speaker)
